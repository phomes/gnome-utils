...
gdict_client_context_error_quark (void)
{
  ...
  if (G_UNLIKELY (quark == 0))
    quark = g_quark_from_static_string ("gdict-client-context-error-quark");
                                   ...
                                   GdictCommand       *command)
{
  ...
  gdict_debug ("Pushing command ('%s') into the queue...\n",
...
gdict_client_context_pop_command (GdictClientContext *context)
{
  ...
  gdict_debug ("Getting command ('%s') from the queue...\n",
                                   ...
                                   GError             **error)
{
  ...
  if (!priv->channel)
    {
      gdict_debug ("No connection established\n");
  ...
  gdict_debug ("Wrote (%d bytes) to the channel\n",
               written_bytes);
                                  ...
                                  GError             **error)
{
  ...
  gdict_debug ("GdictCommand command =\n"
               "{\n"
               "  .cmd_type = '%02d' ('%s');\n"
               "  .database = '%s';\n"
               "  .strategy = '%s';\n"
  ...
  gdict_debug ("Sending command ('%s') to the server\n",
               dict_command_strings[command->cmd_type]);
      ...
      if (priv->status_code == GDICT_STATUS_CONNECT)
      ...
      else
        {
          ...
          g_set_error (&parse_error, GDICT_CONTEXT_ERROR,
                       ...
                       _("Unable to parse the dictionary server reply: '%s'"),
                       buffer);
    ...
    case CMD_SHOW_STRAT:
      if (priv->status_code == GDICT_STATUS_N_STRATEGIES_PRESENT)
      ...
      else
        {
          ...
          /* first token: strategy name
           * second token: strategy description
           */
          tokens = g_strsplit (buffer, " ", 2);
          name = tokens[0];
          if (!name)
    ...
    case CMD_DEFINE:
      if (priv->status_code == GDICT_STATUS_N_DEFINITIONS_RETRIEVED)
      ...
      else if (priv->status_code == GDICT_STATUS_WORD_DB_NAME)
        {
          ...
          tokens = g_strsplit (buffer, " ", 4);
          
          /* we trow away the first token, which stores the status code, and
           * skip to the second token which contains the word we are looking
           * for.
           */
          word = tokens[1];
          if (!word)
...
gdict_client_context_is_connected (GdictClientContext *context)
{
  ...
  if (context->priv->is_connecting == TRUE)
    return TRUE;
